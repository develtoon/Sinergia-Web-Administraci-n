...
(function () {
    ...
    const { Col, Row, Spinner, Modal } = ReactBootstrap
    const { requiredMessage, patterns, minValueMessage, maxValueMessage, tryParseNumber } = Utils
    ...
    let smart = {
        ...
        urlDownloadArchivo: '/DownloadArchivo',
        urlGetSingle: '/GetSingle',
        urlGetCatalogoEstadoList: '/Administracion/CatalogoEstado/GetList',
        urlGetTipoBienList: '/EstudioMercado/TipoBien/GetTipoBienList',
        urlGetSubTipoBienList: '/EstudioMercado/TipoBien/GetSubTipoBienList',
        urlGetSubTipoBienAtributoList: '/EstudioMercado/TipoBien/GetSubTipoBienAtributoList',
        urlGetUbigeoGetList: '/EstudioMercado/Ubigeo/GetList',
    }
    ...
    const ViewIntl = ({ intl }) => {
        ...
        */
                ...
                .then(({ data: resSingle }) => {
                    if (resSingle.apiEstado === 'ok') {
                        ...
                        if (appContext.permisos.esUsuarioSoloConsultar) {
                            ...
                            subTipoBienAtributoBuscar(resSingle.idSubTipoBien, resSingle.atributos, false);
                        }
                        ...
                        setValue('tipoCambio', resSingle.tipoCambio)
                        ...
                        let archivos = resSingle.archivos.map(arc => {
                            return { ...arc, key: generateId() }
                        })
        ...
        const subTipoBienAtributoBuscar = (_id, _atributos, _limpiar) => {
                    ...
                    if (data.apiEstado == "ok") {
                        ...
                        let dataResult = data.data.map(dt => {
                            ...
                            if (_atributos == null) {
                                ...
                                _atributos.filter(atr => atr.id === dt.id)
                                    .map(atr => {
                                        valor = atr.valor;
                                    })
                            }

                            return { ...dt, listado: JSON.parse(dt.listado), valor: valor }
                        })
        ...
        const handleChange = (e) => {
            ...
            let valor = getValues(name);

            if (value == '' || re.test(value)) {
                if (tryParseNumber(value, 2) !== false) {
                    valor = value;
                }
            }
        ...
        const handleBlur = (e) => {
            ...
            if (value == '' || re.test(value)) {
                if (tryParseNumber(value, 4) !== false) {
        ...
        const handleChangeArchivo = (e, _key) => {
            e.preventDefault()
            let name = e.target.name;
            let value = e.target.value;

            let newArrayArchivo = [...dataArchivo]

            let pos = newArrayArchivo.findIndex((item) => item.key === _key);

            newArrayArchivo[pos].descripcion = value;

            setDataArchivo(newArrayArchivo)
        }

        const handleAddArchivo = (files) => {
        ...
        const downloadArchivo = (event, idRegistroArchivo, nombre) => {
            ...
            AXIOS({
                url: smart.urlContext + smart.urlDownloadArchivo, // Interface name
                method: 'get',
                responseType: "blob",
                params: params
            })
        ...
        const handleDeleteFile = (event, _key) => {
        ...
        const handleSave = (data) => {
            ...
            let cantidadArchivo = dataArchivo.length;

            if (cantidadArchivo > 12) {
                swal({
                    title: 'Solo puedes subir 12 fotos como máximo.',
                    // text: data.apiMensaje,
                    icon: "error",
                })
            }
                ...
                let oBien = {
                    ...
                    "tipoCambio": data.tipoCambio ? data.tipoCambio : '0',
        ...
        const handleShowPopUp = (e, _url, _fileName) => {

            setShowModal(true)
            setFileUrlView(_url)
            setFileNameView(_fileName)
            ...
            <div className="o-container c-header__wrapper flex flex-column mt-3">
                    ...
                    <div>
                                            ...
                                            {...register("idSubTipoBien", {
                                                ...
                                                onChange: (e => {
                                                    ...
                                                    if (value === "") {
                                                        ...
                                                        subTipoBienAtributoBuscar(value, null, true);
                                                    }
                                                }),
                                                disabled: allDisabled
                                            })}
                        ...
                        <Row className="mb-2">
                            <div className="mt-2 flex gap-2 justify-space-between align-center w-100">
                                <span className=" title--20 u-text--gray-80 u-text--bold">Datos Específicos</span>
                            </div>
                        </Row>
                        ...
                        <Row className="mb-4">
                            {
                                dataAtributo.map(dtatr => (
                                    <Col key={dtatr.key} sm={12} md={12} lg={3} className="mb-4">

                                        {dtatr.idtipo == 1 && <CInput
                                            mod="mb-2"
                                            label={dtatr.nombre}
                                            name={`input_${dtatr.id}`}
                                            {...register(`input_${dtatr.id}`, {
                                                required: {
                                                    value: dtatr.esobligatorio,
                                                    message: `El campo ${dtatr.nombre.toLowerCase()} es requerido `
                                                },
                                                disabled: allDisabled
                                            })}
                                            onChange={(e) => handleChangeAtributo(e, dtatr.id)}
                                            error={errors[`input_${dtatr.id}`]?.message}
                                            value={dtatr.valor}
                                            requerido={dtatr.esobligatorio ? ("1") : ("0")}
                                            maxLength="200"
                                        />}
                                        ...
                                        {dtatr.idtipo == 4 && <CSelect
                                            ...
                                            {...register(`select_${dtatr.id}`, {
                                                required: {
                                                    value: dtatr.esobligatorio,
                                                    message: `El campo ${dtatr.nombre.toLowerCase()} es requerido `
                                                },
                                                min: {
                                                    value: dtatr.esobligatorio ? (1) : (0),
                                                    message: `El campo ${dtatr.nombre.toLowerCase()} es requerido `
                                                },
                                                disabled: allDisabled
                                            })}
                                            ...
                                            value={dtatr.valor}
                                            requerido={dtatr.esobligatorio ? ("1") : ("0")}
                                            error={errors[`select_${dtatr.id}`]?.message}
                                        />}
                                        ...
                                        {dtatr.idtipo == 2 && <CInput
                                            ...
                                            {...register(`input_${dtatr.id}`, {
                                                ...
                                                maxLength: {
                                                    value: 18,
                                                    message: maxValueMessage('maxlength', 18)
                                                },
                                                min: {
                                                    value: 0,
                                                    message: `El ${dtatr.nombre.toLowerCase()} mínimo(a) es 0`
                                                },
                                                disabled: allDisabled
                                            })}
                            ...
                            <Col lg="4" md="12" sm="12" className="mb-4">
                                <CInput
                                    ...
                                    {...register("tipoCambio", {
                                        ...
                                        min: {
                                            value: 0.01,
                                            message: 'El tipo de cambio mínimo es 0.01'
                                        },
                                        disabled: allDisabled
                                    })}
                                    onChange={(e) => handleChange(e)}
                            ...
                            <div className="mt-2 flex gap-2 justify-space-between align-center w-100">
                                ...
                                {!appContext.permisos.esUsuarioSoloConsultar &&
                                    <CInputFile
                                        ...
                                        onChange={files => handleAddArchivo(files)}
                                        children={'Agregar'}
                                        icon="add_circle"
                                        position="left"
                                        esMultiple={true}
                        ...
                        <div className="mb-2">
                            <Row className="c-card--2 u-shadow u-bg--white" hidden={dataArchivo.length == 0 ? 'hidden' : ('')}>
                                {dataArchivo.map(da => {
                                    return (
                                        <Col lg="3" sm="6" className="mb-3 mt-2" key={da.key} >
                                            <div className="c-file-card--1 c-file-card--responsive u-bg--white mb-2"
                                                style={{ backgroundImage: `url(${da.urlArchivoMiniatura})` }}
                                                onClick={e => handleShowPopUp(e, da.urlArchivo, da.nombreOriginal)}
                                            >
                                                {da.id && <button
                                                    disabled={allDisabled}
                                                    type="button" className="c-button--rounded c-button--rounded--p-3 u-text--white u-bg--green float-start m-2" onClick={(ev) => downloadArchivo(ev, da.id, da.nombre)}>
                                                    <Icon h="21">cloud_download</Icon>
                                                </button>}
                                                <button
                                                    disabled={allDisabled}
                                                    type="button" className="c-button--rounded c-button--rounded--p-3 u-text--white u-bg--red float-end m-2" onClick={(ev) => handleDeleteFile(ev, da.key)}>
                                                    <Icon h="21">close</Icon>
                                                </button>
                                            ...
                                            <CInput
                                                ...
                                                name={`descripcion_${da.key}`}
                                                onChange={(e) => handleChangeArchivo(e, da.key)}
                                                maxLength="200"
                ...
                <Modal show={showModal} onHide={handleCloseModal} size="xl" centered>
                    <Modal.Header bsPrefix="c-modal-header" closeButton>
                        <Modal.Title >
                            <span className="u-text--gray-90 title--30">{fileNameView}</span>
                        </Modal.Title>
                    </Modal.Header>
                    <hr className=" flex-grow-1 u-text--gray-60" />
                    <Modal.Body>
                        <div className="c-file-card--2 c-file-card--responsive u-bg--white mb-2"
                            style={{ backgroundImage: `url(${fileUrlView})` }}
                        ></div>
                    </Modal.Body>

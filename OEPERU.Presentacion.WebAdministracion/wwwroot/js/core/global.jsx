(function () {
    ...
    const AppProvider = ({ children, view }) => {
        ...
        return (
            <AppContext.Provider value={{ userAuth, handleAvatar, breadcumb, menuPermiso, permisos, handleBreadcumb }} >
                {children}
            </AppContext.Provider>
        )
    }
    ...
    const CBreadcrumbs = ({ items }) => {
        return (
            <div className="c-breadcumb d-flex">
                {/* <div className="c-divider c-divider--breadcumb"></div> */}
                <ul className="c-breadcumb__list m-0">
                    {
                        items.map((item, idx) => {
            ...
            <div className={'c-inputfile ' + mod} style={styleInputFile}>
                {
                    <>
                        <input className={`c-inputfile__input `}
                            ...
                            maxLength={maxLength}
                        />
                        <a className={`c-inputfile__button ${isLoading ? 'c-inputfile__button_isLoading' : ''}`} onClick={onClickLink} name={name} style={styleInput}>
                            {
                                isLoading ?
                                    <>
    ...
    const CInput = React.forwardRef(({ min, requerido, id, step, name, mod = '', modInput = '', placeholder, maxLength, label, value = "", icon = "", type = "text", onChange, onBlur,
        onClickIncon, error = "", disabled = false, cRegex = '' }, ref) => {
    ...
    const CSelect = React.forwardRef(({ id, label, onChange, mod = '', value, onBlur, name, options = [], error = "", placeholder = "", requerido = "0", disabled = false }, ref) => {
        return (
            <div className={'c-select ' + mod}>
                ...
                <div className={`c-input__cont-select ${error && 'is-invalid'}`}>
                    <select disabled={disabled} name={name} className="c-select__select" ref={ref} id={id} value={value} onChange={onChange} onBlur={onBlur}>
                        {
                            options.map(option => {
                                return (
    ...
    const CTextArea = React.forwardRef(({ id, name, mod = '', placeholder, maxLength, label, value = "", icon = "", type = "text", onChange, onBlur, error = "", disabled = false, rows = 1 }, ref) => {
        return (
            <div className={'c-input ' + mod}>
                {
                    label ? <label className="c-input__label u-text--light" htmlFor={id}>{label}</label> : ''
                }
                <div className={`c-input__cont-input ${error && 'is-invalid'}`}>
                    {
        ...
        return (
            <div className="c-table__actions flex paragraph paragraph--14 u-text--gray-80">
                <div className="c-table__actions-block-1">
                    ...
                    <select className="browser-default js-pagination__rows" defaultValue={handleChange.perPage} name="number_of_rows" onChange={handleChange}>
                ...
                <div className="flex">
                    <i className="material-icons md-18 js-pagination__prev u-pointer" data-page="p"></i>
    ...
    const CButton = ({ children, isLoading, className, onClick, type = 'button', icon = '', disabled = false }) => {
        return (
            <button
                disabled={disabled}
                type={type} className={`c-button${className ? ' ' + className : ''} ${isLoading ? ' is-loading' : ''} flex align-center`} onClick={onClick}>
                {isLoading ? "" : children}
            </button>
        )
    }
    ...
    function CMenu() {
        ...
        return (
            <nav className="c-nav box flex flex-column">
                {
                    l_menu ?
                        ''
                        :
                                    ...
                                    return (
                                        <div key={menu.id}>
                                            <a className="c-link" href={menu.enlace} target="_self" onClick={(e) => handleShow(menu.id)}>
                                                <li className="c-nav__list-item u-pointer" key={menu.id}>
                                                    <Icon className="" h="30">{menu.icono}</Icon>
                                                </li>
                                            </a>
                                            {menu.visible && <Offcanvas show={show} onHide={handleClose} key={menu.key} >
    ...
    const CustomToggle = React.forwardRef(({ onClick }, ref) => (
        <a className="c-link" onClick={(e) => { e.preventDefault(); onClick(e); }}>
            <Icon children="more_vert" h="20" className="c-dropdown" />
        </a>
    ));
            ...
            <>
                {alter ? <></> :
                    <header className="c-header">
                        ...
                        <div className="o-container c-divider--mx15">
                            <div className="c-divider  c-dividir--gray-20"></div>
                        </div>
